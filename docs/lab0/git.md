...
- Windows 系统：从 <https://git-scm.com/downloads> 的下载页面获取 Git for Windows，按默认选项安装即可
...
/home/cpct        # pwd 的输出。"cpct" 字符串对应的是用户名。整个目录是你的家目录
$ mkdir learn_git # 创建一个新的名为 learn_git 的文件夹
$ cd learn_git    # 进入该文件夹。你可以再次使用 pwd 检查路径
$ git init
Initialized empty Git repository in /home/cpct/learn_git/.git/
```
...
Author: CPCT-TA <fake-email@qq.com>
...
首先你要有个远程仓库，如果你还没有，可以选择在 [GitHub](https://github.com) 或者 [USTC LUG GitLab](https://git.lug.ustc.edu.cn) 或者[科大超算中心 GitLab](https://git.ustc.edu.cn) 上注册账号。
...
```shell
git remote add origin git@github.com:CPCT-TA/learn_git.git
```

请注意把上面的 `CPCT-TA` 替换成你自己的 GitHub 账户名，添加后，远程库的名字为 `origin`，这是 Git 默认的叫法。下一步，就可以把本地库的所有内容推送到远程库上：
...
```shell
$ git clone git@github.com:CPCT-TA/gitskills.git
...
$ git push origin dev
To github.com:CPCT-TA/learn_git.git
 ! [rejected]        dev -> dev (non-fast-forward)
error: failed to push some refs to 'git@github.com:CPCT-TA/learn_git.git'
...
注意： github 单次上传文件有大小限制，如果上传的文件超过 100M，会上传失败 (git push)。
